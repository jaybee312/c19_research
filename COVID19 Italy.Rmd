...
Italy_all = coronavirus %>% filter(Country.Region == "Italy")
Italy_d = Italy_all %>% filter(type == "confirmed")
Italy_g = Italy_all %>% filter(type == "death")

# Summary: overall cases, 
Italy_all = Italy_all %>% group_by(Country.Region, type) %>%
  summarise(total_cases = sum(cases)) 

Italy_all

#mortality rate, recov/dead ratio 
mortality_rate = as.data.frame(Italy_all[2,3] / Italy_all[1,3])
colnames(mortality_rate)[1] = "Mortality Rate"
mortality_rate

recovery_rate = as.data.frame(Italy_all[3,3] / Italy_all[2,3])
...
gr = coronavirus %>% filter(Country.Region == "Italy" & date >= '2020-03-01' & type == "confirmed" ) %>%
  group_by(date) %>% 
...
gr = coronavirus %>% filter(Country.Region == "Italy" & date >= '2020-03-01' & type == "death" ) %>%
  group_by(date) %>% 
  summarise(cases = sum(cases)) %>% mutate(diff = 1 + (cases - lag(cases))/ lag(cases)) %>% arrange(desc(date))


ggplot(gr, aes(x=date, y=cases), ) + geom_point() + geom_smooth() +
  labs(title = "Italy Death Growth",
       x = "Date",
       y = "New Cases")
